...
// The absolute base URL for links in outgoing email, or null when it cannot be established safely. APP_URL IS REQUIRED IN PRODUCTION. The development fallback below reads the request's own Host / X-Forwarded-Host, and both are supplied by whoever made the request — see the header. On a developer's own machine there is no victim, and requiring configuration there would mean the reset flow never gets tested locally.
