...
const summarySearchSpan = 10;

// Excel serial 0. Workbooks using the 1904 system shift this — see parse().
const excelEpoch1900 = Date.UTC(1899, 11, 30);
const excelEpoch1904 = Date.UTC(1904, 0, 1);
...
export function excelValueToMinutes(v: CellLike, date1904 = false): number | null {
  ...
  if (u instanceof Date) {
    ...
    const mins = Math.round((u.getTime() - epoch) / msPerMin);
...
function readPeriod(
  ...
  const monthCell = unwrap(ws.getCell(rowMonth, colLabel).value);
  const headerYear = asNumber(ws.getCell(rowYear, colLabel).value);
...
const fallbackCountOrder = ['P', 'T', 'LM', 'S', 'OH', 'L', 'CO', 'HD', 'WO'];
...
function readCountHeaders(ws: ExcelJS.Worksheet, bandStart: number): string[] {
  ...
  for (let i = 0; i < summaryCountColumns; i++) {
    headers.push(normalizeStatus(asText(ws.getCell(rowDayNumbers, bandStart + i).value)));
  }
...
export async function parseRegisterWorkbook(buf: ArrayBuffer | Buffer): Promise<ParsedRegister> {
  ...
  const colWorkingDays = bandStart + offsetWorkingDays;
  const colPayableDays = bandStart + offsetPayableDays;
  ...
  for (let r = blockStartRow; r <= lastRow; r += blockStride) {
    const idRaw = asNumber(ws.getCell(r, colEmplId).value);
    ...
    const inRow = findLabelledRow(ws, r + 1, r + blockStride - 1, /^IN\b/);
  ...
  if (employees.length === 0) {
    throw new Error(
      `No employee blocks found. Expected an Empl. ID in column A starting at row ${blockStartRow}, every ${blockStride} rows.`,
    );
  }
