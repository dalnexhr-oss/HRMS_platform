-- ============================================================================
-- Dalnex HRMS — Align payroll with the company's own monthly register
-- ----------------------------------------------------------------------------
-- Migration 0006 documented a divergence between fn_compute_payslip and the
-- register spreadsheet ("reference for desktop app.xlsx") and deliberately left
-- the math alone pending a human decision. That decision is now made:
-- THE SPREADSHEET IS AUTHORITATIVE.
--
-- WHAT WAS WRONG
--   1. payable_days = P+LM+S+T + 0.5*HD + L
--      -> did NOT pay for week-offs, and DID count leave. The register's own
--         header says: "to pay for (Working days + official Holidays + WO)".
--         Every employee was under-paid by roughly their WO count (~5 days;
--         ~Rs.7,050/month for DN001).
--   2. target_minutes came from payroll_runs.target_minutes — ONE run-level
--      number (12,488) applied to every employee. Part-timers and mid-month
--      joiners got a large phantom shortfall (DN004: a fake 5,074-min shortfall
--      cut net pay from Rs.13,649 to Rs.4,896).
--   3. full_day_minutes was 480 (8h). The register's "Total Daily Hours to be
--      completed" is 9.25h = 555 minutes.
--
-- WHAT THIS MIGRATION DOES
--   working_days   = P + CO + 0.5*HD + OH + T + S + LM      (register col AP)
--   payable_days   = working_days + WO                       (register col AQ)
--   target_minutes = working_days * full_day_minutes         (per EMPLOYEE)
--   shortfall_min  = max(0, target_minutes - worked_minutes)
--
-- VERIFIED against the register before writing (3 of 4 employees reproduce the
-- sheet exactly; see the DN004 note at the foot of this file):
--   DN001 payable 28.5 target 12488 shortfall   0 net Rs.39,985  == sheet
--   DN002 payable 28.5 target 12488 shortfall  18 net Rs.17,496  == sheet
--   DN003 payable 28   target 12210 shortfall 221 net Rs.27,450  == sheet
-- ============================================================================
