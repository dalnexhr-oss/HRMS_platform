 ...
 *
 * punched_at is timestamptz, so a bare `>= '2026-08-24T00:00:00'` is read as
...
export async function readPunchStatus(): Promise<PunchStatus> {
  ...
  const todays = (events.data ?? []).filter(
    (event) => localParts(new Date(event.punched_at)).date === today,
  );
...
function sumWorkedMinutes(events: DayEvent[]): number {
  ...
  for (const event of events) {
    const clock = localParts(new Date(event.punched_at)).time;
    if (event.kind === 'in') {
...
export async function recordPunch(
  ...
  const { data: priorRaw, error: priorError } = await supabase
    .from('punch_events')
    .select('kind, punched_at')
    .eq('employee_id', employeeId)
  ...
  const prior = (priorRaw ?? []).filter(
    (event) => localParts(new Date(event.punched_at)).date === date,
  );
  ...
  const { error: eventError } = await supabase.from('punch_events').insert({
...
async function resolveDay(
  ...
  const times = events.map((event) => ({
    kind: event.kind,
    clock: localParts(new Date(event.punched_at)).time,
  }));
